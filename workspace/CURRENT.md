...
Last refreshed: 2026-08-28 12:06 Australia/Brisbane.
...
- PR #3: open/draft against `develop`; latest-head CI observed green during this calibration.
...
- PR #3 body is stale relative to current STATUS/ROADMAP because it still describes an earlier pending cold-load/canary gate.
...
- no open PR observed during this calibration; latest `develop` CI observed green.

Current owning-repo status:
- pre-alpha; the bounded S1 behavior smoke and thin Platform Policy Adapter are source/test complete.
...
- Human Gold is still absent; scientific Core/B6/final-v0 claims remain blocked.
...
- `develop`: `386489e16a94d265c7eef0547df63af179a17c4a`
- historical bootstrap branch head: `17399174605fb94d94d2b45314928b1905ae87fe`
- initial governance has already been activated on `main`; the earlier description that main was only initialized at `fed1fd7...` is superseded.
- Skill/Governance reconciliation is performed through a new short-lived topic branch from current `develop`.

Canonical Skill source/deployment identity is tracked by `skills/SKILL_SUITE_MANIFEST.json`; canonical complete Skill release/source archives now live under `skills/releases/<skill-name>/skill.zip` and are checked by `scripts/validate_skill_suite.py`.
